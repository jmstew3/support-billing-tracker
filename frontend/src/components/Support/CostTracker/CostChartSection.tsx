...
import type { MonthlyCostData, CostData } from '../CostTrackerCard';

export interface CostChartSectionProps {
  /** Monthly breakdown data (for multi-month view) */
  monthlyCosts?: MonthlyCostData[];
  /** Single period cost data (for single-month view) */
  costData?: CostData;
  /** Selected month */
  selectedMonth: number | 'all';
  /** Visible urgencies filter */
...
/**
 * CostChartSection - Renders Recharts visualization for cost data
 *
 * Handles both multi-month and single-period chart rendering with
 * interactive urgency filtering.
 */
export function CostChartSection({
  ...
  // Render monthly chart (stacked bars by month)
        ...
        <ResponsiveContainer width="100%" height={400}>
          <ComposedChart data={chartData}>
            ...
            <Legend
              ...
              iconType="rect"
              content={(props) => renderCustomLegend(props.payload)}
            />
  ...
  // Render single period chart (grouped bars by urgency)
  ...
  // Render custom legend with clickable urgency filters
  const renderCustomLegend = (payload: any) => {
    const customOrder = ['Promotion', 'Low', 'Medium', 'High'];
    const orderedPayload = customOrder.map(key =>
      payload?.find((item: any) => item.value === key)
    ).filter((item: any): item is NonNullable<typeof item> => item !== null && item !== undefined);

    return (
      <div>
        <ul style={CHART_STYLES.legendList}>
          {orderedPayload.map((entry: any, index: number) => entry ? (
            <li
              key={`item-${index}`}
              onClick={() => onToggleUrgency(entry.value || '')}
              style={{
